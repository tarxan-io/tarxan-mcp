...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "deploy",
            description: "Trigger a deploy action via NATS",
            inputSchema: {
                type: "object",
                properties: {
                ...
                required: ["user_id", "template_id", "creds"],
            },
        },
        {
            name: "delete",
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "deploy") {
            ...
            const payload = DeploySchema.parse(data);
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Deploy event published for user ${payload.user_id}`,
                    },
                ],
            };
        }
